...
export function useConnectors() {
	...
	const sortedConnectors = React.useMemo(() => {
		...
		const formattedConnectors = connectors.reduce<Array<Connector>>(
			(acc, curr) => {
				switch (curr.id) {
					case "metaMaskSDK":
						metaMaskConnector = {
		...
		const numInjectedConnectors = formattedConnectors.filter(
			({ type }) => type === "injected",
		).length;
		const allConnectors =
			injectedConnector && numInjectedConnectors <= 1
				? [injectedConnector, ...formattedConnectors]
				: formattedConnectors;
		if (
			metaMaskConnector &&
			!allConnectors.find(
